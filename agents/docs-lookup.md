<!-- REQUIRES INTERNET: This agent uses external services -->

# Agent: docs-lookup

> **Tier**: internet
> **Model tier**: mid (map to your best local model if tier=best, mid otherwise)
> **Tools needed**: file_read, search, mcp__context7__resolve-library-id, mcp__context7__query-docs
...
The harness may expose Context7 tools under prefixed names (e.g. `documentation lookup tool`, `documentation lookup tool`). Use the tool names available in your environment (see the agent’s `tools` list).

### Step 1: Resolve the library

Call the Context7 MCP tool for resolving the library ID (e.g. **resolve-library-id** or **documentation lookup tool**) with:
...
Call the Context7 MCP tool for querying docs (e.g. **query-docs** or **documentation lookup tool**) with:
...
Action: Call the resolve-library-id tool (e.g. documentation lookup tool) with libraryName "Next.js", query as above; pick `/vercel/next.js` or versioned ID; call the query-docs tool (e.g. documentation lookup tool) with that libraryId and same query; summarize and include middleware example from docs.
