...
> **Tier**: local
> **Model tier**: best (map to your best local model if tier=best, mid otherwise)
> **Tools needed**: file_read, search, file_search, shell_exec, file_edit, file_write
...
- **Rules are system-injected**: `.the AI assistant/rules/*.md` files load automatically every session. Unlike prompt instructions, the LLM cannot choose to ignore them.

## Example Invocations

```bash
the AI assistant /mail                    # Email-only triage
the AI assistant /slack                   # Slack-only triage
the AI assistant /today                   # All channels + calendar + todo
the AI assistant /schedule-reply "Reply to Sarah about the board meeting"
```

## Prerequisites

- [AI coding assistant](https://docs.anthropic.com/en/docs/the AI assistant-code)
