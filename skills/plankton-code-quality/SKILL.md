...
Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for AI coding assistant. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns the AI assistant subprocesses to fix violations the agent didn't catch.
...
Every time AI coding assistant edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
├─ Spawns the AI assistant -p subprocess with violations JSON
...
# Start AI coding assistant — hooks activate automatically
the AI assistant
```

No install command, no plugin config. The hooks in `.the AI assistant/settings.json` are picked up automatically when you run AI coding assistant in the Plankton directory.

### Per-Project Integration

To use Plankton hooks in your own project:

1. Copy `.the AI assistant/hooks/` directory to your project
2. Copy `.the AI assistant/settings.json` hook configuration
...
Plankton's `.the AI assistant/hooks/config.json` controls all behavior:
