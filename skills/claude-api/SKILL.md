...
Build applications with the Anthropic the AI assistant API and SDKs.

## When to Activate

- Building applications that call the the AI assistant API
- Code imports `anthropic` (Python) or `@anthropic-ai/sdk` (TypeScript)
- User asks about the AI assistant API patterns, tool use, streaming, or vision
- Implementing agent workflows with the AI assistant Agent SDK
...
| Model | ID | Best For |
|-------|-----|----------|
| Opus 4.6 | `{{BEST_MODEL}}` | Complex reasoning, architecture, research |
| Sonnet 4.6 | `{{MID_MODEL}}` | Balanced coding, most development tasks |
| Haiku 4.5 | `{{FAST_MODEL}}` | Fast responses, high-volume, cost-sensitive |
...
Define tools and let the AI assistant call them:
...
export ANTHROPIC_MODEL="{{MID_MODEL}}"
```
