...
# Configure Everything AI coding assistant (ECC)

An interactive, step-by-step installation wizard for the Everything AI coding assistant project. Uses `ask user` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure ecc", "install ecc", "setup everything AI coding assistant", or similar
...
This skill must be accessible to AI coding assistant before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install everything-the AI assistant-code` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.the AI assistant/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc"
...
```bash
rm -rf /tmp/everything-the AI assistant-code
git clone https://github.com/affaan-m/everything-the AI assistant-code.git /tmp/everything-the AI assistant-code
```

Set `ECC_ROOT=/tmp/everything-the AI assistant-code` as the source for all subsequent copy operations.

If the clone fails (network issues, etc.), use `ask user` to ask the user to provide a local path to an existing ECC clone.

---

## Step 1: Choose Installation Level

Use `ask user` to ask the user where to install:

```
Question: "Where should ECC components be installed?"
Options:
  - "User-level (~/.the AI assistant/)" — "Applies to all your AI coding assistant projects"
  - "Project-level (.the AI assistant/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=~/.the AI assistant`
- Project-level: `TARGET=.the AI assistant` (relative to current project root)
- Both: `TARGET_USER=~/.the AI assistant`, `TARGET_PROJECT=.the AI assistant`
...
There are 7 selectable category groups below. The detailed confirmation lists that follow cover 45 skills across 8 categories, plus 1 standalone template. Use `ask user` with `multiSelect: true`:
...
Options:
  ...
  - "Research & APIs" — "Deep research, Exa search, the AI assistant API patterns"
...
For each selected category, print the full list of skills below and ask the user to confirm or deselect specific ones. If the list exceeds 4 items, print the list as text and use `ask user` with an "Install all listed" option plus "Other" for the user to paste specific names.
...
| `the AI assistant-api` | Anthropic the AI assistant API patterns: Messages, streaming, tool use, vision, batches, Agent SDK |
...
Use `ask user` with `multiSelect: true`:
...
```bash
grep -rn "~/.the AI assistant/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.the AI assistant/` paths:
- If a skill references `~/.the AI assistant/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `~/.the AI assistant/skills/` or `~/.the AI assistant/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.the AI assistant/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.the AI assistant/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .the AI assistant/skills/")
...
### "Skills not being picked up by AI coding assistant"
- Verify the skill directory contains a `SKILL.md` file (not just loose .md files)
- For user-level: check `~/.the AI assistant/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.the AI assistant/skills/<skill-name>/SKILL.md` exists
...
- Restart AI coding assistant after installing rules

### "Path reference errors after project-level install"
- Some skills assume `~/.the AI assistant/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.the AI assistant/homunculus/` directory is always user-level — this is expected and not an error.
