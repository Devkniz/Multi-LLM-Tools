...
- **Prompts**: Reusable, parameterised prompt templates the client can surface (e.g. in the AI assistant Desktop). Register with `registerPrompt()` or equivalent.
- **Transport**: stdio for local clients (e.g. the AI assistant Desktop); Streamable HTTP is preferred for remote (Cursor, cloud). Legacy HTTP/SSE is for backward compatibility.
